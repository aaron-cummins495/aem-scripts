...
for (def prefixFolder : profilesFolderChildren) {
    ...
    for (def cfFolder : prefixFolder.getChildren()) {
        ...
        if (cfNode.hasProperty("jcr:content/data/master/authorizedAdminsMigrated")) {
            ...
            // Remove "( and )" from the string if they exist
            authorizedAdminsMigrated = authorizedAdminsMigrated.replaceAll("\"(", "\"")
            authorizedAdminsMigrated = authorizedAdminsMigrated.replaceAll(")\"", "\"")
        ...
        if (cfNode.hasProperty("jcr:content/data/master/areasOfSpecializationMigrated")) {
            def areasOfSpecializationMigrated = cfNode.getProperty('jcr:content/data/master/areasOfSpecializationMigrated').getString()
            
            // Assuming areasOfSpecializationMigrated is a pipe-separated list of specialization names
            def specializationNames = areasOfSpecializationMigrated.split("\\|")
            def propertiesNode = getNode("${cfNode.path}/jcr:content/data/master")
            println("Migrated specializations for ${propertiesNode.path}: ${specializationNames}")

            // Write String[] prop to node
            propertiesNode.setProperty("areasOfSpecialization", specializationNames)
            resourceResolver.commit()
        }
...
for (def prefixFolder : profilesFolderChildren) {
    ...
    for (def cfFolder : prefixFolder.getChildren()) {
        ...
        if (cfNode.hasProperty("jcr:content/data/master/authorizedAdminsMigrated")) {
            ...
            // Assuming authorizedAdminsMigrated is a pipe-separated list of admin names
            def adminNames = authorizedAdminsMigrated.split("\\|")
            //println("Migrated admins for ${cfResource.name}: ${adminNames}")

            // Write String[] prop to node
            def targetNode = getNode("${cfNode.path}/jcr:content/data/master")
            println(targetNode.path)
            targetNode.setProperty("authorizedAdmins", adminNames)
        }
        
        session.workspace.copy(cfReszurce.path, newPath.toString())
